...
export function Render2(html,styleMap){
 let items = [
      {id: 0, name: 'J'},
      {id: 1, name: 'S'},
      {id: 2, name: 'K'},
...
let span_0=html`<span @click=${this.taks} class="mod" style="bottom:0px;position: relative;">${!a?iy_text:ix_text}</span>`;
...
<div class="super_header">${n>=3?this.Left(Left,span_atr(this._role[2]),span_u2):null}
<header  class="header">

<div id="1count" class="player1CardsContainer">${Header}</div>
...
</header>${n===4?this.Right(Right,span_atr(this._role[3]),span_u3):null}</div>
...
<div id="0count" class="player0CardsContainer">${Footer}</div>
</div>
</footer>
</div>`
